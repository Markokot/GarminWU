...
- Цели: ${user.goals || "не указаны"}`;
  ...
  if (user.garminWatch) {
    ...
    if (!supportsSwimStructured) {
      context += ` (ВАЖНО: эти часы НЕ поддерживают плавательные тренировки через Garmin Connect вообще — это аппаратное ограничение, Garmin не позволяет загружать тренировки по плаванию на эту модель. Кнопка "На Garmin" для плавания покажет предупреждение. Для плавания: создавай workout_json как обычно (для Intervals.icu и сохранения в избранное), но ОБЯЗАТЕЛЬНО предупреди пользователя, что на эти часы плавательные тренировки НЕ загружаются — нужно открыть план на экране телефона и выполнять по нему вручную, запустив обычную активность "Плавание в бассейне" на часах.)`;
    }
