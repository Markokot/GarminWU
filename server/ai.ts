...
- Цели: ${user.goals || "не указаны"}`;
  ...
  if (user.garminWatch) {
    ...
    if (!supportsSwimStructured) {
      context += ` (НЕ поддерживает структурированные плавательные тренировки с repeat-блоками — для плавания создавай тренировку БЕЗ repeat-блоков: используй только простые шаги (warmup, interval, recovery, cooldown) последовательно, а подробный план с интервалами опиши текстом в поле "description". Тренировка ДОЛЖНА быть в формате workout_json — пользователь отправит её на часы кнопкой. НИКОГДА не предлагай пользователю копировать JSON, импортировать вручную или работать с Garmin Connect напрямую — у нас есть кнопка отправки)`;
    }
