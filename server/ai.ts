...
- Цели: ${user.goals || "не указаны"}`;
  ...
  if (user.garminWatch) {
    ...
    if (!supportsSwimStructured) {
      context += ` (Эти часы НЕ поддерживают структурированные плавательные тренировки — Garmin Connect отклонит ЛЮБУЮ плавательную тренировку с интервалами. Для плавания на этих часах создавай workout_json с ОДНИМ единственным шагом: warmup (durationType "lap.button", targetType "no.target"). Массив steps должен содержать ровно ОДИН элемент. Весь подробный план тренировки (разминка, интервалы, дистанции, темп, отдых, заминка) ОБЯЗАТЕЛЬНО опиши ПОЛНОСТЬЮ в поле "description" текстом. Пользователь прочитает план с экрана телефона и выполнит вручную. Тренировка ДОЛЖНА быть в формате workout_json — пользователь отправит её на часы кнопкой.)`;
    }
